...
using RegistrationSystem.Helpers;
        ...
        public HttpResponseMessage Get()
        {
            if (CanHandleAuthentication(this.Request) == true)
            {
                    ...
                    expectPassword == Password)
                {
                    ...
                    // 帳號與密碼比對正確，回傳帳密比對正確
                    response = this.Request.CreateResponse(HttpStatusCode.OK, new APIResult()
                    {
                        Success = true,
                ...
                else
                {
                    ...
                    response = this.Request.CreateResponse(HttpStatusCode.Unauthorized, new APIResult()
                    {
                        Success = false,
                        Message = $"",
                        Payload = "帳號或密碼不正確"
